...
let filter_icon_buttons = document.querySelectorAll(
  ".filter_icon_buttons button"
);
...
let alignment_icon_buttons = document.querySelectorAll(
  ".alignment_icon_buttons button"
);
let rotate = 0,
  Flip_X = 1,
  Flip_Y = 1;
...
applyCropButton.addEventListener("click", () => {
  ...
  if (!isNaN(x) && !isNaN(y) && !isNaN(width) && !isNaN(height)) {
    ...
    ctx.drawImage(imgSrc, x, y, width, height, 0, 0, width, height);
